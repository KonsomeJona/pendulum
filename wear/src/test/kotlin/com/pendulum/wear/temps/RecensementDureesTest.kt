...
class RecensementDureesTest {
     ...
     *
     * Deux familles, et une seule regle pour les distinguer : le **temps capteur** ne se comprime
     * pas. La source synthetique du banc conserve la periode nominale en temps capteur tout en
     * defilant vite en temps mural ; une fenetre exprimee en nanosecondes de `SensorEvent` mesure
     * donc exactement la meme chose sur le banc et au poignet, et la diviser lui ferait mesurer
     * autre chose.
     */
    private val tolerances = mapOf(
        "PreviewEnvelope.kt:1_000_000_000L" to
            "seau d'une seconde de temps capteur, pas de temps mural",
        "StopConditions.kt:30_000_000_000L" to
            "epoque de 30 s de WakeDetector, en nanosecondes de SensorEvent",
    )
     ...
     * test vert. `wear` est le nom du repertoire de module a chercher depuis la racine.
     */
    ...
    private fun racine(): File {
        ...
        repeat(4) {
            val essai = File(candidat, "wear/src/main/kotlin")
