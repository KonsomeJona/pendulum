...
| Aritake-Okada S. et al. **Ankle actigraphy against polysomnography for periodic limb movements.** *Sleep Medicine* 2014, n = 41 | **identifier not recorded — to be verified** | *not read by this project* | Cited in `06-interface.md` §"the 15/h legend" for three figures that are used there: a matching threshold closer to **16/h** when measured at the ankle by actigraphy, an **ICC of 0.90 from three nights** above 15/h, and **26 nights** required below it. The 16/h value is a named constant in the code. **This entry exists to record a gap, not to vouch for one**: the paper was cited without ever being listed here, and neither its identifier nor its access status is known. Either it is identified and read, or the three figures it supports must be qualified in `06-interface.md` |
