 ...
 * never have been added here at all. Same test as `:wear`'s, copied rather than shared: the two
 * modules are two applications, with no common source set to put it in.
 */
class DurationsTest {
    ...
    fun `the catalogue is not empty`() {
        ...
        assertThat(accessors()).hasSizeGreaterThanOrEqualTo(6)
    }
    ...
    fun `the active catalogue follows the compiled variant`() {
        assertThat(Durations.ACTIVE.readGiveUpMs)
            .isEqualTo(Durations(TimeScaling.DIVISOR).readGiveUpMs)
    }
}
